...
async def scrape_products(page) -> dict:
    """Extract products and listing metadata from the archive page.

    Returns a dict with:
      - products: list of product card dicts (newest sort, page 1 only)
      - total_products: total item count across all pages (parsed from
        the listing's "Showing N out of M products" indicator)
      - total_pages: number of pagination pages (parsed from the
        pagination control)

    The two count fields act as backup signals for the new-product diff:
    Skins drops items in big batches that don't always hit page 1 of
    newest-sort, but they always change the totals.
    ...
    result = await page.evaluate(
        ...
        () => {
            ...
            // --- Total product count ---
            // The listing wrapper has data-aria-live-text="Showing 24 out of 526 products"
            // and also <meta itemprop="numberOfItems" content="526">. Try the meta first
            // since it's more stable.
            let total_products = null;
            const meta = document.querySelector('[itemprop="numberOfItems"]');
            if (meta) {
                const v = parseInt(meta.getAttribute('content'), 10);
                if (!isNaN(v)) total_products = v;
            }
            if (total_products === null) {
                const wrapper = document.querySelector('[data-aria-live-text]');
                if (wrapper) {
                    const text = wrapper.getAttribute('data-aria-live-text') || '';
                    const m = text.match(/out of (\\d+)/i);
                    if (m) total_products = parseInt(m[1], 10);
                }
            }

            // --- Total page count ---
            // Pagination control has links like ?p=2, ?p=3, etc. The highest p value
            // is the total page count. We scan all anchors for href containing "p=N".
            let total_pages = null;
            const pageNumbers = new Set();
            // Look in pagination-area first for tighter scoping
            const paginationLinks = document.querySelectorAll(
                '.pagination a, [class*="pagination"] a, nav a[href*="p="]'
            );
            const linksToScan = paginationLinks.length > 0
                ? paginationLinks
                : document.querySelectorAll('a[href*="p="]');
            for (const a of linksToScan) {
                const href = a.getAttribute('href') || '';
                const m = href.match(/[?&]p=(\\d+)/);
                if (m) pageNumbers.add(parseInt(m[1], 10));
            }
            if (pageNumbers.size > 0) {
                total_pages = Math.max(...pageNumbers);
            }
            // Sanity fallback: if we have totals but no page link, infer from product count.
            // Skins uses 24 per page.
            if (total_pages === null && total_products !== null) {
                total_pages = Math.ceil(total_products / 24);
            }

            return { products: results, total_products, total_pages };
        }
        """
    )
    return result
...
async def run() -> None:
    ...
    if STATE_FILE.exists():
        try:
            ...
            raw_state = []
        # Backward compat: state.json used to be a flat list of products.
        # New format is a dict with products + counts. Detect old format and
        # treat as "no count history yet" so first run after upgrade doesn't
        # spuriously alert about the page count "appearing from nothing".
        ...
        old_state = {"products": [], "total_products": None, "total_pages": None}
        first_run = True
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            scrape_result = await scrape_products(page)
            products = scrape_result["products"]
            total_products = scrape_result.get("total_products")
            total_pages = scrape_result.get("total_pages")
        finally:
            try:
    ...
    old_total_products = old_state.get("total_products")
    old_total_pages = old_state.get("total_pages")

    old_ids = {item["id"] for item in old_products}
    new_products = [p for p in products if p["id"] not in old_ids]

    # Count-change detection. Fires whenever the totals move in either
    # direction, but suppressed on first run (no baseline) or when either
    # value couldn't be parsed (probably a scrape glitch — better to skip
    # one round than send a misleading "0 → 526" alert).
    count_changed = False
    products_delta = None
    pages_delta = None
    if (
        not first_run
        and old_total_products is not None
        and old_total_pages is not None
        and total_products is not None
        and total_pages is not None
    ):
        if total_products != old_total_products or total_pages != old_total_pages:
            count_changed = True
            products_delta = total_products - old_total_products
            pages_delta = total_pages - old_total_pages

    # Persist new state. Structured format: products + counts.
    ...
    if first_run:
        await send_telegram_message(
            ...
            f"You'll get a ping here whenever new items appear or counts change.\n\n"
            f"<i>{now_str()}</i>"
        )
    else:
        # Build combined alert: count-change banner (if any), then individual
        # new-product cards (if any). Send count alert first as a single
        # message, then the per-product pings via notify_new.
        if count_changed:
            arrow_p = "↑" if products_delta > 0 else "↓"
            arrow_pg = "↑" if pages_delta > 0 else ("↓" if pages_delta < 0 else "→")
            sign_p = f"+{products_delta}" if products_delta > 0 else str(products_delta)
            sign_pg = f"+{pages_delta}" if pages_delta > 0 else (
                str(pages_delta) if pages_delta < 0 else "0"
            )
            extra = ""
            if new_products:
                extra = (
                    f"\n\n{len(new_products)} of these appear on page 1 "
                    f"of newest-sort — details below."
                )
            else:
                extra = (
                    "\n\n<i>None of the changes show up on page 1 of newest-sort. "
                    "May be a batch drop landing on later pages, or restocks/"
    ...
    if should_heartbeat and not first_run:
        ...
        await send_telegram_message(
            ...
            f"products across "
            f"{total_pages if total_pages is not None else '?'} pages.\n\n"
            f"<i>{now_str()}</i>"
        )

    print(
        f"OK. Total={len(products)} New={len(new_products)} "
        f"Pages={total_pages} Items={total_products} "
        f"CountChanged={count_changed} "
